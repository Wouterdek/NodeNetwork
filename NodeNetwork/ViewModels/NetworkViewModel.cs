...
namespace NodeNetwork.ViewModels
{
	...
	/// </summary>
	[DataContract]
	public class NetworkViewModel : ReactiveObject, IHaveId
	{
		...
		[IgnoreDataMember] private double _left;
		...
		[IgnoreDataMember] private double _top;

		/// <summary>
		/// Gets or sets the current connections for Serialisation.
		/// </summary>
		/// <value>The current connections.</value>
		[DataMember] public IEnumerable<ConnectionViewModel> CurrentConnections { get; set; }

		/// <summary>
		/// Gets or sets the current inputs for Serialisation.
		/// </summary>
		/// <value>The current inputs.</value>
		[DataMember] public IEnumerable<NodeInputViewModel> CurrentInputs { get; set; }

		/// <summary>
		/// Gets or sets the current nodes for Serialisation.
		/// </summary>
		/// <value>The current nodes.</value>
		[DataMember] public IEnumerable<NodeViewModel> CurrentNodes { get; set; }

		/// <summary>
		/// Gets or sets the current outputs for Serialisation.
		/// </summary>
		/// <value>The current outputs.</value>
		[DataMember] public IEnumerable<NodeOutputViewModel> CurrentOutputs { get; set; }
		#endregion
		...
		/// </summary>
		[IgnoreDataMember] public ISourceList<NodeViewModel> Nodes { get; } = new SourceList<NodeViewModel>();
		#endregion
		...
		/// </summary>
		[IgnoreDataMember] public IObservableList<NodeViewModel> SelectedNodes { get; }
		#endregion
		...
		/// </summary>
		[IgnoreDataMember] public ISourceList<ConnectionViewModel> Connections { get; } = new SourceList<ConnectionViewModel>();
		#endregion
		...
		[IgnoreDataMember] private PendingConnectionViewModel _pendingConnection;
		#endregion
		...
		[IgnoreDataMember] private NodeViewModel _pendingNode;
		#endregion
		...
		[IgnoreDataMember] private Func<NodeInputViewModel, NodeOutputViewModel, ConnectionViewModel> _connectionFactory;
		#endregion
		...
		[IgnoreDataMember] private Func<NetworkViewModel, NetworkValidationResult> _validator;
		#endregion
		...
		[IgnoreDataMember] private NetworkValidationResult _latestValidation;
		#endregion
		...
		/// </summary>
		[IgnoreDataMember] public IObservable<NetworkValidationResult> Validation { get; }
		#endregion
		...
		public bool IsReadOnly
		{
			...
			set
			{
				//Set All nodes to read only
				if (CurrentNodes != null)
				{
					foreach (var node in CurrentNodes)
					{
						node.IsReadOnly = value;
					}
				}
				this.RaiseAndSetIfChanged(ref _isReadOnly, value);
			}
		}
		[IgnoreDataMember] private bool _isReadOnly;
		#endregion
		...
		{
			get => _isZoomEnabled;
			set => this.RaiseAndSetIfChanged(ref _isZoomEnabled, value);
		}
		[IgnoreDataMember] private bool _isZoomEnabled;
		#endregion
		...
		/// </summary>
		[DataMember] public CutLineViewModel CutLine { get; } = new CutLineViewModel();
		#endregion
		...
		/// </summary>
		[DataMember] public SelectionRectangleViewModel SelectionRectangle { get; set; } = new SelectionRectangleViewModel();
		#endregion
		...
		/// </remarks>
		[IgnoreDataMember] public IObservable<Unit> ConnectionsUpdated { get; }
		...
		/// </summary>
		[IgnoreDataMember] public IObservable<Unit> NetworkChanged { get; }
		#endregion
		...
		/// </summary>
		[IgnoreDataMember] public ReactiveCommand<Unit, Unit> DeleteSelectedNodes { get; }

		/// <summary>
		/// Runs the Validator function and stores the result in LatestValidation.
		/// </summary>
		[IgnoreDataMember] public ReactiveCommand<Unit, NetworkValidationResult> UpdateValidation { get; }
		#endregion
		...
		public NetworkViewModel()
		{
			...
			ConnectionFactory = (input, output) => new ConnectionViewModel(this, input, output) { Id = Guid.NewGuid().ToString() };
			...
			// Get recoverable items for serialization
			Nodes.Connect().ToCollection().Subscribe(list => {
				CurrentNodes = list.AsEnumerable();
				var nivm = new List<NodeInputViewModel>();
				var novm = new List<NodeOutputViewModel>();
				var expectedCount = 0;
				foreach (var item in list)
				{
					expectedCount += item.Inputs.Count;
					item.Inputs.Connect().ToCollection().Subscribe(ilist => {
						var x = ilist.AsEnumerable();
						nivm.AddRange(x);
					});
					expectedCount += item.Outputs.Count;
					item.Outputs.Connect().ToCollection().Subscribe(ilist => {
						var x = ilist.AsEnumerable();
						novm.AddRange(x);
					});
				}
				CurrentInputs = nivm.AsEnumerable();
				CurrentOutputs = novm.AsEnumerable();
				var res = (CurrentInputs.Count() + CurrentOutputs.Count()) == expectedCount;
				if (res)
				{
					Console.WriteLine("de-serialised Count Correct");
				}
			});
			Connections.Connect().ToCollection().Subscribe(list => CurrentConnections = list.AsEnumerable());
